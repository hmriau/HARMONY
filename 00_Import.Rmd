...
#save who is included for consort diagram
w12_id_complete <- harmony_wk12 %>% 
  #filter(week_12_checkin_complete==2) %>% 
  dplyr::select(record_id,redcap_event_name,redcap_data_access_group,week_12_checkin_complete)
write.csv(w12_id_complete,here("Data","w12_id_complete.csv"))
...
#save who is included for consort diagram
w24_id_complete <- harmony_wk24 %>% 
  #filter(week_24_interview_cati_complete==2) %>% 
  dplyr::select(record_id,redcap_event_name,redcap_data_access_group,week_24_interview_cati_complete)
write.csv(w24_id_complete,here("Data","w24_id_complete.csv"))
...
Compare the imported datasets with what I received from client for consort diagram

```{r}
#Week 12
#read in the data I received to compare
wk12_inclusions <- read.csv(here("Data","Original","LIVEHARMONY-WK12AndDropOut_DATA_2025-07-22_1607.csv")) %>%
  mutate(redcap_include_flag = 1)
#flag the data I received for analysis
w12_id_complete <- w12_id_complete %>% mutate(import_include_flag = 1)
#join the two
w12_ids <- full_join(wk12_inclusions,w12_id_complete,by="record_id")
write.csv(w12_ids,here("Data","w12_ids_compare.csv"))

#Week 24
#read in the data I received to compare
wk24_inclusions <- read.csv(here("Data","Original","LIVEHARMONY-WK24Retention_DATA_2025-07-22_1607.csv")) %>%
  mutate(redcap_include_flag = 1)
#flag the data I received for analysis
w24_id_complete <- w24_id_complete %>% mutate(import_include_flag = 1)
#join the two
w24_ids <- full_join(wk24_inclusions,w24_id_complete,by="record_id")
write.csv(w24_ids,here("Data","w24_ids_compare.csv"))
```

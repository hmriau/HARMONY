...
pacman::p_load(here,tidyverse,gtsummary,brms,mice,emmeans,marginaleffects,epitools)
...
#calculate the IRR of AEs and SAEs
# Calculate IRR and confidence interval
saes <- rateratio(matrix(c(4,8,241,259),2,2))
aes <- rateratio(matrix(c(64,84,241,259),2,2))
#sae IRR = 1.83 (95%CI 0.56, 7.08)
#ae IRR = 1.22 (95%CI 0.88, 1.70)

#(8/259) / (4/241) for IRR = 1.86
#sae / N in group
#(84/259) / (64/241) for IRR = 1.22
#ae / N in group
...
adhere_dat <- adhere_dat %>%
  dplyr::select(record_id,oralnrt_any_w12,oralnrt_any_w24,vape_any_w12,vape_any_w24,
                patchany_w12,patchany_w24,
                redcap_data_access_group,atop_cannabis_used_baseline,treatment,.imp)

#pivot longer the dataset
nrt <- adhere_dat %>% 
  dplyr::select(record_id,oralnrt_any_w12,oralnrt_any_w24,patchany_w12,patchany_w24,.imp) %>%
  mutate(adherencenrt_w12 = case_when(oralnrt_any_w12 == 0 | patchany_w12 == 0 ~ 1,
                                   oralnrt_any_w12 %in% c(1,2) & patchany_w12 %in% c(1,2) ~ 0),
         adherencenrt_w24 = case_when(oralnrt_any_w24 == 0 | patchany_w24 == 0 ~ 1,
                                   oralnrt_any_w24 %in% c(1,2) & patchany_w24 %in% c(1,2) ~ 0)) %>%
  pivot_longer(cols = c(adherencenrt_w12,adherencenrt_w24),
               names_to = "time", values_to = "adherencenrt") %>%
  mutate(time = factor(case_when(time == "adherencenrt_w12" ~ "week12",
                          time == "adherencenrt_w24" ~ "week24"))) %>%
  dplyr::select(-oralnrt_any_w12,-oralnrt_any_w24,-patchany_w12,-patchany_w24)
...
adhere_dat <- adhere_dat %>% merge(nrt,by=c(".imp","record_id")) %>%
  ...
  mutate(redcap_data_access_group = factor(redcap_data_access_group),
         treatment = factor(treatment),
         record_id = factor(record_id),
         adherence = case_when(adherencenrt == 1 & treatment == "NRT" ~ 1,
                               vape_any == "Yes, currently using" & treatment == "VNP" ~ 1,
                               adherencenrt == 0 & treatment == "NRT" ~ 0,
...
raw_adhere <- harmony %>% 
  filter(redcap_event_name %in% c("week_12_arm_1","week_24_arm_1")) %>% 
  group_by(redcap_event_name,treatment) %>% 
  mutate(adherence = case_when(oralnrt_any == 0 | patchany == 0 & treatment == "NRT" ~ 1,
                               vape_any == "Yes, currently using" & treatment == "VNP" ~ 1,
                               oralnrt_any != 0 & !is.na(oralnrt_any) & patchany != 0 & !is.na(patchany) & treatment == "NRT" ~ 0,
                               vape_any != "Yes, currently using" & !is.na(vape_any) & treatment == "VNP" ~ 0)) %>%
